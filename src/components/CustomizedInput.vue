        ...
        <div class="w-100 d-flex flex-column gap align-items-center" style="position: relative;">
            ...
            <div class="w-100 d-flex flex-row gap align-items-center">
                ...
                <div class="text-area w-100" v-if="props.type == 'textarea'">
                    ...
                    width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="8" cy="8" r="7.71429" fill="white" stroke="#FF500B" stroke-width="0.571429" />
                        <path
                ...
                <input v-else :class="props.type == 'number' ? 'input-actions' : ''" :type="props.type"
                    class="customized-input__input epilogue-regular" :key="props.keyRender"
                    :placeholder="getPlaceHolder(props)" v-model="inputValue" @input="emitInput"
                    :style="props.options ? 'border-radius:0px' : ''" />
                <span v-if="props.showActions && props.type != 'textarea'" class="input-actions " @click="() => inputValue++
                    "> +</span>
            </div>
            ...
            <div v-if="props.options" class="selector">
                ...
                <span v-for="(option, index) in props.options"
                    :style="option.value == selected || option.selected == true ? 'background-color:red;color:white' : ''"
                    class="options" @click="selectOption(option.value)">{{ option.text }}
